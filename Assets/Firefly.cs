...
public class Firefly : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // --- ส่วนที่เพิ่มเข้ามา ---
        // ตั้งเวลาทำลายตัวเองโดยเรียกใช้ฟังก์ชัน DestroyFirefly หลังจากผ่านไป lifespan วินาที
        // เราจะย้ายการเรียก Invoke ไปไว้ท้าย Coroutine FadeInEffect
        // เพื่อให้แน่ใจว่า lifespan เริ่มนับ *หลังจาก* fade in เสร็จสมบูรณ์
        // Invoke("DestroyFirefly", lifespan); // <--- ย้ายไปไว้ใน Coroutine
        // -------------------------
    }
    ...
    // --- ฟังก์ชันใหม่สำหรับทำลาย GameObject ---
    private void DestroyFirefly()
    {
        // อาจจะเพิ่มเอฟเฟกต์ Fade out หรือ Particle ก่อนทำลายก็ได้
        // ตัวอย่าง: StartCoroutine(FadeOutAndDestroy());

        // ทำลาย GameObject ที่ Script นี้อยู่
        Destroy(gameObject);
    }
    // ----------------------------------------

    // (Optional) Coroutine สำหรับ Fade out ก่อนทำลาย
    /*
    IEnumerator FadeOutAndDestroy(float duration = 0.5f)
    {
        float timer = 0f;
        Color startColor = spriteRenderer.color;
        float startIntensity = (fireflyLight != null) ? fireflyLight.intensity : 0f;
        ...
        while(timer < duration)
        {
            ...
            if(fireflyLight != null)
            {
                fireflyLight.intensity = Mathf.Lerp(startIntensity, 0f, progress);
            }
            timer += Time.deltaTime;
            yield return null;
        }
    ...
    */
    public void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            if (globalPoint != null)
            {
                globalPoint.flowerPoints += 3;
                Debug.Log("Firefly points: " + globalPoint.flowerPoints);
            }
